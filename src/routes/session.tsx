    ...
    <AppShell>
      <div className="mx-auto max-w-3xl space-y-6 p-5 md:p-8">
        <div className="flex items-center justify-between">
          <div>
            <div className="font-mono text-xs uppercase tracking-widest text-muted-foreground">JAMB Biology · Lesson 1 of 5</div>
        ...
        <AnimatePresence mode="wait">
          {phase === "lesson" && (
            <motion.div key="lesson" initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -16 }} className="rounded-3xl border border-border bg-card p-8">
              <p className="text-lg leading-relaxed">{LESSON}</p>
              <div className="mt-8 flex items-center gap-3">
                <button onClick={togglePlay} className="btn-press inline-flex items-center gap-2 rounded-full bg-accent px-5 py-3 text-sm font-medium text-accent-foreground">
                  {playing ? (
                ...
                <button onClick={() => { stopSpeaking(); setPlaying(false); }} className="rounded-full border border-border px-4 py-3 text-sm">Stop</button>
                <button onClick={() => { stopSpeaking(); setPhase("quiz"); }} className="ml-auto rounded-full px-4 py-3 text-sm text-muted-foreground hover:text-foreground">Skip to quiz →</button>
              </div>
            </motion.div>
          )}

          {phase === "quiz" && (
            <motion.div key={qi} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="rounded-3xl border border-border bg-card p-8">
              <div className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Question {qi + 1} / {QUESTIONS.length}</div>
              ...
              <div className="mt-6 grid gap-3">
                {QUESTIONS[qi].opts.map((o, i) => {
                  ...
                  return (
                    ...
                    >
                      <span className="font-mono mr-3 text-xs opacity-70">{String.fromCharCode(65 + i)}</span>{o}
                    </motion.button>
                  );
                })}
              </div>
          ...
          {phase === "done" && (
            <motion.div key="done" initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} className="rounded-3xl border border-border bg-card p-10 text-center">
              <div className="font-mono text-xs uppercase tracking-widest text-muted-foreground">Result</div>
              <div className="mt-3 font-display text-6xl">{Math.round((score / QUESTIONS.length) * 100)}%</div>
              <p className="mt-3 text-muted-foreground">You got {score} of {QUESTIONS.length} right.</p>
              <div className="mt-8 flex flex-wrap justify-center gap-3">
                <Link to="/dashboard" className="btn-press rounded-full border border-border bg-background px-5 py-3 text-sm">Back to Dashboard</Link>
                <button onClick={() => { setPicks([]); setQi(0); setPhase("lesson"); }} className="btn-press rounded-full bg-accent px-5 py-3 text-sm font-medium text-accent-foreground">Next Lesson</button>
              </div>
            </motion.div>
          )}
        </AnimatePresence>
