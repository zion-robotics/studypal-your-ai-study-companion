  ...
  return (
    <div className="relative grid h-56 w-56 place-items-center">
      <svg viewBox="0 0 220 220" className="absolute inset-0 -rotate-90">
        ...
        <circle cx="110" cy="110" r={r} stroke="currentColor" className="text-accent" strokeWidth="16" fill="none" strokeDasharray={c} strokeDashoffset={offset} strokeLinecap="round" style={{ transition: "stroke-dashoffset 1.2s cubic-bezier(.2,.8,.2,1)" }} />
      </svg>
...
function Dashboard() {
  ...
  useEffect(() => { const t = setTimeout(() => setPct(70), 100); return () => clearTimeout(t); }, []);
  ...
  const badge = (s: number) => s >= 85 ? "bg-emerald-500/15 text-emerald-500" : s >= 70 ? "bg-amber-500/15 text-amber-500" : "bg-red-500/15 text-red-500";
    ...
    <AppShell>
      <div className="mx-auto max-w-6xl space-y-6 p-5 md:p-8">
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Link to="/session" className="btn-press rounded-full bg-accent px-5 py-2.5 text-sm font-medium text-accent-foreground">Start today's session</Link>
        </motion.div>

        {/* Daily Pulse */}
        <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05, duration: 0.5 }} className="grid gap-6 rounded-3xl border border-border bg-card p-6 md:grid-cols-[auto,1fr] md:p-8">
          ...
          <div className="flex flex-col justify-between gap-6">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="rounded-xl border border-border p-4">
                <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Streak</div>
                <div className="mt-1 font-display text-2xl">7 days</div>
              </div>
              <div className="rounded-xl border border-border p-4">
                <div className="font-mono text-[10px] uppercase tracking-widest text-muted-foreground">Syllabus</div>
        ...
        <div className="grid gap-6 md:grid-cols-[1.3fr,1fr]">
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1, duration: 0.5 }} className="rounded-3xl border border-border bg-card p-6 md:p-8">
            ...
            <div className="mt-2 flex flex-wrap items-end justify-between gap-4">
              <div>
                ...
                <div className="text-sm text-muted-foreground">Topic 8: Cellular Respiration · est. 12 min</div>
              </div>
              <Link to="/session" className="btn-press rounded-full bg-accent px-5 py-2.5 text-sm font-medium text-accent-foreground">Start Session</Link>
            </div>
            <div className="mt-6">
              <div className="mb-2 flex justify-between font-mono text-[10px] uppercase tracking-widest text-muted-foreground">
                <span>Plan progress</span><span>Lesson 12 / 38</span>
              </div>
          ...
          <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15, duration: 0.5 }} className="rounded-3xl border border-border bg-card p-6">
        ...
        {/* Quick stats */}
        <motion.section initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2, duration: 0.5 }} className="grid grid-cols-2 gap-4 md:grid-cols-4">
